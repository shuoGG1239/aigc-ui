...
| 类型 | 来源 | 可编辑 |
|------|------|--------|
| 用户池 | `userData/prompt_pools/*.json` | 是 |
| 内置 JSON | `web/src/random/prompt_pools/*.json`（Vite embed） | 条目可覆写到 userData；不可删/改名 |
| **程序池** | `web/src/random/program-pools.ts` | **否**；列表可见（标「程序」），详情黑盒；仅 `<pool:name>` → 字符串 |

当前：`act_common` / `act_common2` / `act_jk`。
...
- **换行一律 LF**：仓库 `.gitattributes`（`eol=lf`）+ `.editorconfig` + `.vscode/settings.json`（`files.eol: \n`）；Git 全局/本地 `core.autocrlf=false`、`core.eol=lf`。Agent/工具写文件不得产出 CRLF
...
| `web/src/random/program-pools.ts` | 不透明 `<pool:act_*>` 抽样（内部闭环） |
